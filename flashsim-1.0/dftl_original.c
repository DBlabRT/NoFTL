...
/******************************DFTL PARAMS for BLOCK DEVICE************************************/

static int block_device_cache_level_1 = 6552;
module_param(block_device_cache_level_1, int, 0);
MODULE_PARM_DESC(block_device_cache_level_1, "DFTL for block device: Max number of entries in CACHE LEVEL 1");

static int block_device_cache_level_2 = 1640;
module_param(block_device_cache_level_2, int, 0);
MODULE_PARM_DESC(block_device_cache_level_2, "DFTL for block device: Max number of entries in CACHE LEVEL 2");

static int block_device_num_extra_blocks = 256; //3%
module_param(block_device_num_extra_blocks, int, 0);
MODULE_PARM_DESC(block_device_num_extra_blocks, "DFTL for block device: Number of extra blocks!");

/******************************DFTL PARAMS for CHAR DEVICE************************************/

static int char_device_cache_level_1 = 500;
module_param(char_device_cache_level_1, int, 0);
MODULE_PARM_DESC(char_device_cache_level_1, "DFTL for char device: Max number of entries in CACHE LEVEL 1");

static int char_device_cache_level_2 = 100;
module_param(char_device_cache_level_2, int, 0);
MODULE_PARM_DESC(char_device_cache_level_2, "DFTL for char device: Max number of entries in CACHE LEVEL 2");

static int char_device_num_extra_blocks = 256; //3%
module_param(char_device_num_extra_blocks, int, 0);
MODULE_PARM_DESC(char_device_num_extra_blocks, "DFTL for char device: Number of extra blocks!");
...
static int write_sector_oob_field(struct dftl_s *dftl, int psn, int field_idx, oob_field field_value, unit_type_e unit_type, int is_gc, int simulate_delay)
{
	...
	retval = access_sector_oob(dftl->params.device_type, psn, &field_value, (field_idx*sizeof(oob_field)), sizeof(oob_field), WRITE, simulate_delay);
	if (retval) {
...
static int read_sector_oob(struct dftl_s *dftl, int psn, oob_field *sector_oob, unit_type_e unit_type, int is_gc, int simulate_delay)
{
	...
	retval = access_sector_oob(dftl->params.device_type, psn, sector_oob, 0, sizeof(oob), READ, simulate_delay);
	if (retval) {
...
static int write_sector_oob(struct dftl_s *dftl, int psn, oob_field *sector_oob, unit_type_e unit_type, int is_gc, int simulate_delay)
{
	...
	retval = access_sector_oob(dftl->params.device_type, psn, sector_oob, 0, sizeof(oob), WRITE, simulate_delay);
	if (retval) {
...
static int read_sector(struct dftl_s *dftl, int psn, void * buffer, unit_type_e unit_type, int is_gc)
{
	...
	if (dftl->params.device_type == BLOCK_DEVICE)
		copy_from_flashsim_internal(dftl->block_device, buffer, psn);
	else {
		offset = (loff_t)psn * (loff_t)dftl->params.dev_params->sector_size;
		char_device_read_internal(dftl->char_device, buffer, dftl->params.dev_params->sector_size, &offset);
	}
...
static int write_sector_with_oob (struct dftl_s *dftl, int psn, int lsn, const void *src, unit_type_e unit_type, int is_gc)
{
	...
	//write data
	if (dftl->params.device_type == BLOCK_DEVICE)
		copy_to_flashsim_internal(dftl->block_device, src, psn);
	else {
		offset = (loff_t)psn * (loff_t)dftl->params.dev_params->sector_size;
		char_device_write_internal(dftl->char_device, src, dftl->params.dev_params->sector_size, &offset);
	}
...
static int erase_block(struct dftl_s *dftl, int block_idx)
{
	...
	if (dftl->params.dev_params->device_type == BLOCK_DEVICE)
		retval = erase_block_internal(&dftl->block_device->dev_core, block_idx);
	else
		retval = erase_block_internal(&dftl->char_device->dev_core, block_idx);
...
static int add_sector_to_batch_map_update(struct dftl_s *dftl, struct list_head *batch_update_store, int lsn, int new_psn)
{
	...
	if (!map_sec_update) {
		...
		map_sec_update = kmalloc(sizeof(struct map_sector_update), GFP_ATOMIC);
		if (!map_sec_update) {
	...
	mapping_update = kmalloc(sizeof(struct mapping_update), GFP_ATOMIC);
	if (!mapping_update) {
		print_error("Memory allocation failed!");
		return -ENOMEM;
	}
...
struct dftl_s * init_dftl (void * device, int device_type, int *out_num_data_sectors)
{
	...
	if (device_type == BLOCK_DEVICE) {
		dftl->block_device = (struct flashsim_block_device *)device;
		dftl->params.dev_params = &dftl->block_device->dev_core.params;
		dftl->params.cmt_real_max_entries = block_device_cache_level_1;
		dftl->params.cmt_ghost_max_entries = block_device_cache_level_2;
		//TODO add parameter check here: (num_extra_blocks > num_blocks) && (num_extra_blocks < THRESHOLD + 1)
		dftl->params.num_extra_blocks = block_device_num_extra_blocks;
	}
	else if (device_type == BLOCK_DEVICE) {
		dftl->char_device = (struct flashsim_char_device *)device;
		dftl->params.dev_params = &dftl->char_device->dev_core.params;
		dftl->params.cmt_real_max_entries = char_device_cache_level_1;
		dftl->params.cmt_ghost_max_entries = char_device_cache_level_2;
		dftl->params.num_extra_blocks = char_device_num_extra_blocks;
	}
	else {
		print_error("Invalid device type!");
		return NULL;
	}
	...
	dftl->sector_buffer.buffer = vzalloc(dftl->params.dev_params->sector_size);
	dftl->sector_buffer.fixed = 0;
	...
	*out_num_data_sectors = dftl->params.num_data_sectors;
	return dftl;
}
