...
public:
		...
		w_rc_t nsm_create_file();
		w_rc_t pax_create_file();
		w_rc_t hpl_create_file();
 ...
 */
rc_t
smthread_user_t::nsm_create_file() 
{
...
/// and zeros for data 
	...
	TimeMeasure timer("timemeasure.nsm.csv");
	timer.startTimer();
	...
	{
		{
			...
			o << "00001000azobichammachiboza1234567" << ends;
			w_assert1(o.c_str() == dummy);
		}
		// header contains record #
		int i = j;
		const vec_t hdr(&i, sizeof(i));
		W_COERCE(ssm->create_rec(info.fid, hdr, _rec_size, data, rid));
#if HPL_DEBUG>1
...
rc_t
smthread_user_t::pax_create_file() 
{
	file_info_t info;  // will be made persistent in the
	// volume root index.
	flid_t f;
	...
	// init precord
	short number_of_fields = 3;
	short number_of_fixed_fields = 2;
	precord_t prec(number_of_fields, number_of_fixed_fields);

	// initialize record structure
	int fieldCounter=0;
	for (fieldCounter=0; fieldCounter < number_of_fixed_fields; ++fieldCounter)
		prec.field_size[fieldCounter] = sizeof(int);
	for (; fieldCounter < number_of_fields; ++fieldCounter)
		prec.field_size[fieldCounter] = 0;
	...
	// Create the file. Stuff its fid in the persistent file_info
	W_DO(ssm->create_file(_vid, info.fid, smlevel_3::t_regular, prec));		

	rid_t rid;

	_rec_size -= align(sizeof(int));

	TimeMeasure timer("timemeasure.pax.csv");
	timer.startTimer();
	...
	for(int j=0; j < _num_rec; j++)
	{
		...
		prec.data = new char*[number_of_fields];
		int i=0;
		for (i=0; i < number_of_fixed_fields; ++i)
		{
			prec.field_size[i] = sizeof(int);
			prec.data[i] = new char[prec.field_size[i]];
			memcpy(prec.data[i], &i, prec.field_size[i]);
		}
		for (; i < number_of_fields; ++i)
		{
			char text[] = "azobichammachiboza1234567";
			int realTextSize = strlen(text);
			prec.field_size[i] = realTextSize;
			prec.data[i] = new char[realTextSize];
			memcpy(prec.data[i], text, realTextSize);
		}
		...
		cout << "Creating rec #" << j << endl;
#endif
		W_COERCE(ssm->create_rec(info.fid, prec, rid));
	...
	timer.stopTimer();
	cout << "Created all for "<< (float)timer.getElapsedTime()/1000 <<"ms. First rid " << info.first_rid << endl;
	timer.printData();
...
rc_t
smthread_user_t::hpl_create_file()
{
	file_info_t info;  // will be made persistent in the
	...
	short number_of_fields = 3;
	short number_of_fixed_fields = 2;
	hpl_record_t record(number_of_fields, number_of_fixed_fields);

	// initialize record structure
	int fieldCounter=0;
	for (fieldCounter=0; fieldCounter < number_of_fixed_fields; ++fieldCounter)
		record.fieldSize[fieldCounter] = sizeof(int);
	for (; fieldCounter < number_of_fields; ++fieldCounter)
		record.fieldSize[fieldCounter] = 0;

	// create and fill file to scan
	cout << "Creating a file with " << _num_rec << " records of size " << _rec_size << endl;
	W_DO(ssm->begin_xct());

	W_DO(ssm->create_file(_vid, info.fid, smlevel_3::t_regular, record));
	...
	TimeMeasure timer("timemeasure.hpl.csv");
	timer.startTimer();


	for ( int j=1; j < _num_rec; j++ )
	{
		record.data = new char*[number_of_fields];
		int i=0;
		for (i=0; i < number_of_fixed_fields; ++i)
		{
			record.fieldSize[i] = sizeof(int);
			record.data[i] = new char[record.fieldSize[i]];
			int valueOfField = j+j*i;
			memcpy(record.data[i], &valueOfField, record.fieldSize[i]);
		}
		for (; i < number_of_fields; ++i)
		{
			char text[] = "azobichammachiboza1234567";
			int realTextSize = strlen(text);
			record.fieldSize[i] = realTextSize;
			record.data[i] = new char[realTextSize];
			memcpy(record.data[i], text, realTextSize);
		}
		...
		cout << "----------------------------------------------\n" << "Creating rec #" << j << endl;
#endif
		W_COERCE(ssm->create_rec(info.fid, record, rid));
...
#if HPL_DEBUG>1
	cout << "Read inserted records!" << endl;
#endif
	int errors=0;
	for ( int j=1; j < 513; j++ )
	{
		...
		cout << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n" << "Reading rec #" << j << endl;
#endif
		rid_t get_rid = info.first_rid;
		get_rid.slot = info.first_rid.slot+j;
		record.fieldSize[2]=12345;
		W_COERCE(ssm->read_hpl_rec(get_rid,record));
		if (!strcmp(record.data[2], "azobichammachiboza1234567")==0)
			errors++;
	}
	...
	rid_t del_rid = info.first_rid;
	del_rid.slot = info.first_rid.slot+200;
#if HPL_DEBUG>1
	cout << "Delete record with rid.slot " << del_rid.slot << endl;
#endif
	W_COERCE(ssm->destroy_hpl_rec( del_rid));


#if HPL_DEBUG>1
	cout << "Update record with rid.slot: " << del_rid.slot <<" and field: "<< 1 << endl;
	W_COERCE(ssm->read_hpl_rec( del_rid,record));
#endif
//	int valueOfField = 62345;
//	Size2 dataSize = sizeof(int);
	char* valueOfField = "blabla123";
	Size2 dataSize = strlen(valueOfField);
	W_COERCE(ssm->update_hpl_field( del_rid, 2, static_cast<const char*>(static_cast<const void*>(valueOfField)), dataSize));
#if HPL_DEBUG>1
	W_COERCE(ssm->read_hpl_rec( del_rid,record));
#endif
	W_COERCE(ssm->update_hpl_field( del_rid, 2,NULL, 0));
...
smthread_user_t::do_init()
{
	...
	switch (PAGE_LAYOUT) {
		case 0: //"NSM"
			cout <<"create NSM file " <<endl;
			W_DO(nsm_create_file());
		break;
		case 1: //"PAX"
			cout <<"create PAX file " <<endl; 
			W_DO(pax_create_file());
		break;
		case 2: //"NSM"
			cout <<"create HPL file " <<endl;
			W_DO(hpl_create_file());
		break;
		default:
			cout <<"create default NSM file " <<endl;
			W_DO(nsm_create_file());	// NSM
		break;
	}
