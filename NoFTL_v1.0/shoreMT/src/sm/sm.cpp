...
hpl_m* 		smlevel_2::hpl_man = 0; // HPL PAGE
pfile_m*  	smlevel_2::pfi = 0; 	// PAX page
...
rc_t ss_m::setup_options(option_group_t* options)
{
	...
	//Hardock (default changed from "yes" -> "no")
	W_DO(options->add_option("sm_backgroundflush", "yes/no", "no",
...
ss_m::~ss_m()
{
	...
	_destruct_once();
	if (FTL_MODE) {
		sector_mapping_ftl::close_ftl();
		sector_mapping_ftl::print_statistics();
	}
...
ss_m::_destruct_once()
{
	...
	delete hpl_man; hpl_man = 0; // hpl manager
	delete pfi; pfi = 0; // pax manager
