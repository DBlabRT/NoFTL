...
sthread_base_t::fileoff_t& vol_t::get_offset(fileoff_t &offset, int rw, int device_fd)
{
    if (FTL_MODE)
    	W_COERCE_MSG(sector_mapping_ftl::translate_file_pos(offset, rw, device_fd), << "FTL failed!");
    return offset;
}
...
vol_t::read_page(shpid_t pnum, page_s& page)
{
    ...
    w_rc_t err = t->pread(_unix_fd, (char *) &page, sizeof(page), get_offset(offset, 0, _unix_fd));
...
vol_t::write_many_pages(shpid_t pnum, const page_s* const pages, int cnt)
{
    ...
    for (int i = 0; i < cnt; i++) {
		fileoff_t offset = (fileoff_t(pnum) + i) * sizeof(page_s);
		W_COERCE_MSG(t->pwrite(_unix_fd, &(pages[i]), sizeof(page_s),
				get_offset(offset, 1, _unix_fd)), << "volume id=" << vid());
    }
    ...
    bool                skip_raw_init)
{
    ...
    {
    	//Cursor for writing pages. Needed here to avoid all calls of lseek, write
    	//and only usage of pwrite
        int write_page_idx = 1;
         ...
         */
        {
            ...
            cout << "num_exts:" << num_exts << endl;
            extlink_p ep(buf, st_regular);
            extnum_t i;
            extlink_t link;
            for (i = 0; i < num_exts; i += ep.max, ++epid.page, write_page_idx++)  {
                ...
                fileoff_t offset = fileoff_t(write_page_idx)*sizeof(page);
                rc = me()->pwrite(fd, &page, sizeof(page), get_offset(offset, 1, fd));
...
//            cout << "k: " << k << endl;
//            rc = me()->lseek(fd, ((k+1)*sizeof(page_s)), sthread_t::SEEK_AT_SET);
        }
        ...
        { 
            ...
            for (i = 0; i < num_exts; i += fp.max, spid.page++, write_page_idx++)  {
                DBGTHRD(<<"");
...
//Hardock: we do not need this non-official copy of volume header.
//In case of nand device we could save additional write
//    if (!raw_device) {
//        /*
...
//    }
     ...
     */
    fileoff_t offset = 0;
    W_DO(me()->pwrite(fd, tmp, tmpsz, get_offset(offset, 1, fd) + sector_size));
...
vol_t::read_vhdr(int fd, volhdr_t& vhdr)
{
     ...
     */
    fileoff_t offset = 0;
    W_DO(me()->pread(fd, tmp, tmpsz, get_offset(offset, 0, fd) + sector_size));
