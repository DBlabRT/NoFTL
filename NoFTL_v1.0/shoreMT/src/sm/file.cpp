...
)
{
    ...
    W_DO(_create_rec(fid, t_append, len_hint,sd, hdr, data, rid, page));
 ...
 */
rc_t
file_m::_create_rec(const stid_t& fid, pg_policy_t policy, smsize_t len_hint, sdesc_t& sd,
		const vec_t& hdr, const vec_t& data, rid_t& rid, file_p& page // in-output
)
{
...

	{
		...
		rec_impl = file_p::choose_rec_implementation(hdr.size(), est_data_len, space_needed);
	...
	{ // open scope for hu
		...
		if (page.is_fixed())
		{
			w_assert2(policy == t_append); w_assert2(page.latch_mode() == LATCH_EX);
			...
			if (rc.is_error())
			{
				...
				} w_assert3(!page.is_fixed()); w_assert3(!have_page);
			}
			else
			{
...
#if W_DEBUG_LEVEL > 2
				{
					...
					if((page.page_bucket_info.old() != b) && page.page_bucket_info.initialized())
					{
		...
		if (!have_page)
		{
			W_DO(_find_slotted_page_with_space(fid, policy, sd,	space_needed, page, slot));
			hu.replace_page(&page);
		}
...
rc_t file_m::_find_slotted_page_with_space(const stid_t& stid, pg_policy_t mask, // might be t_append
		...
		sdesc_t& sd, smsize_t space_needed, file_p& page, // output
		slotid_t& slot // output
		)
{
	uint4_t policy = uint4_t(mask);

	// cout<<"Store ID: "<< stid.store << endl;
	...
	if (policy & t_cache)
	{
		...
		while (!found)
		{
			...
			// used to cripple the pages
//			if ((stid.store == 6 && slot >= 320) ||
//					(stid.store == 9 && slot >= 28) ||
//					(stid.store == 13 && slot >= 384) ||
//					(stid.store == 15 && slot >= 1856) ||
//					(stid.store == 18 && slot >= 576) ||
//					(stid.store == 22 && slot >= 320) ||
//					(stid.store == 25 && slot >= 290) ||
//					(stid.store == 28 && slot >= 63) ) //table Order Line
//			{
//				//cout << "BREAK -->" << page.pid()<< "-" << slot<< endl;// do nothing ( breaks the while loop)
//				break;
//			}

			if (found)
			{
				//cout << page.pid() << "-" << slot<< endl;
				w_assert2(page.is_fixed());
	...
	if (policy & t_compact)
	{
		...
		if (found)
		{
			...
			while (!eof)
			{
				...
				{
					W_DO(
							h->latch_lock_get_slot( lpid.page, &page, space_needed, false, // not append-only
							found, slot));
					if (found)
					{
						// used to cripple the pages
...
//								(stid.store == 22 && slot >= 320) ||
//								(stid.store == 25 && slot >= 290) ||
//								(stid.store == 28 && slot >= 63))
//						{
//							//cout << "compact: BREAK " << slot << endl;
//							eof = true;
//						}
//						else
//						{
//							//cout << "compact: " << slot << endl;
//						}

						w_assert3(page.is_fixed()); DBG(<<"found page " << page.pid().page << " slot " << slot);
				...
				W_DO(
						next_page_with_space(lpid, eof, file_p::free_space2bucket(space_needed) + 1));
	...
	w_assert3(!found); w_assert3(!page.is_fixed());
...
//					(stid.store == 28 && slot >= 63))
		...
		{
			DBG(<<"look in last page - which is " << lastpid ); w_assert2(io->is_valid_page_of(lastpid, stid.store));
			...
			W_DO(h->latch_lock_get_slot( lastpid.page, &page, space_needed, !may_search_file, /* append-only */	found, slot));
			if (found)
			{
		...
		w_assert3(page.is_fixed()); w_assert3(page.latch_mode() == LATCH_EX);
		...
		W_DO(
				h->latch_lock_get_slot( newpid.page, &page, space_needed, !may_search_file, found, slot));

		if (found)
		{
			w_assert3(page.is_fixed()); DBG(<<"found page " << page.pid().page
					<< " slot " << slot);
...
rc_t
file_p::format(const lpid_t& pid, tag_t tag, uint4_t flags, store_flag_t store_flags)
{
