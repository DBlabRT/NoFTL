...
{
	w_assert3(HeapProperty(0)); DBGTHRD(<<"Search starting at " << i
	...
	if (cmp.ge(elements[parent], t))
	{
		...
		while (child < numElements)
		{
			...
			else
			{
				...
				if ((child < numElements) && (cmp.ge(elements[child], t)))
				{
					parent = child;
...
ostream &operator<<(ostream &o, const SearchableHeap<pginfo_t,
		histoid_compare_t> &sh)
{
	sh.Print(o);
	return o;
}
...
#endif
typedef class w_hash_i<histoid_t, occ_rwlock, stid_t>
		w_hash_i_histoid_t_stid_t_iterator;
	...
	cmp(s), refcount(0)
{
	DBGTHRD(<<"create histoid_t for store " << s
			<< " returns this=" << this);
	pgheap = new SearchableHeap<pginfo_t, histoid_compare_t> (cmp,
			pages_in_heap);
...
histoid_t::~histoid_t()
{
	DBGTHRD(<<"destruct histoid_t for store " << cmp.key
			<< " destroying this=" << this); w_assert3(!_in_hash_table()); w_assert1(refcount == 0);
	delete pgheap;
	pgheap = 0;
}
...
void histoid_t::destroy_table()
{
...

	{
		...
		while ((h = iter.next()))
		{
			...
			if (gotit)
			{
				w_assert1(h->refcount == 0); DBGTHRD(<<"removing " << *h); w_assert1(h->_in_hash_table()); w_assert1( htab->num_members() >= 1);
				htab->remove(h);
...
void histoid_t::_victimize(int howmany)
{
	...
	while ((h = iter.next()))
	{
		...
		if (gotit)
		{
			...
			{
				DBGTHRD(<<"deleting " << h); w_assert1( htab->num_members() >= 1);
...
static bool histo_pin_if_pinned(int volatile* refcount)
{
	...
	while (1)
	{
		if (old_pin_cnt == 0)
			return false;
		int new_pin_cnt = atomic_cas_32((unsigned*) refcount, old_pin_cnt,
				old_pin_cnt + 1);
...
//
histoid_t* histoid_t::copy() const
{
...
histoid_t* histoid_t::acquire(const stid_t& s)
{
	...
	if (h)
	{
		...
		{
			h->_grab_mutex();
			w_assert1(h->refcount >= 0); DBGTHRD(<<"incr refcount for " << s
					<< " from " << h->refcount);
	...
	else
	{
		...
		h = new histoid_t(s);
		w_assert2(h->refcount == 0); w_assert2(!h->_in_hash_table());
		...
		h->_release_mutex();
	} w_assert2(h); w_assert2(h->refcount >= 1); DBGTHRD(<<"acquired, store=" << s
			<< " this=" << h
			<< " refcount= " << h->refcount
	);
	return h;
...
void histoid_t::_insert_store_pages(const stid_t& s)
{
	DBGTHRD(<<"_insert_store_pages histoid " << this << " for store " << s); w_assert3(_in_hash_table()); w_assert3(_have_mutex()); w_assert3(pgheap->NumElements() == 0); w_assert3(pgheap->HeapProperty(0));
	...
	DBGTHRD(<<"_insert_store_pages  " << this << " for store " << cmp.key ); DBGTHRD(<<"io_m scan found " << numpages << " pages" );
	...
	for (int i = 0; i < numpages; i++)
	{
		DBGTHRD(<<"page index." << i << "=pid " << pages[i].page()
				<< " space is " << pages[i].space()); DBGTHRD(<<"pgheap.AddElementDontHeapify page "
				<<pages[i].page() << " space " << pages[i].space());
		pgheap->AddElementDontHeapify(pages[i]); // makes a copy
	}
	pgheap->Heapify();
	w_assert3(pgheap->NumElements() == numpages); w_assert3(_have_mutex());
}

bool histoid_t::release()
{
...
void histoid_t::destroyed_store(const stid_t&s, sdesc_t*sd)
{
	...
	while (!success)
	{
		...
		if (h)
		{
			...
			if (success)
			{
				...
				w_assert1( h->_in_hash_table()); T(__LINE__, htab->num_members(), h); w_assert1( htab->num_members() >= 1);
				htab->remove(h); // h->link.detach(), decrement _cnt
				T(__LINE__, htab->num_members(), h); w_assert2( !h->_in_hash_table());
				h->_release_mutex();
			}
		}
		else
		...
		if (h)
		{
			w_assert1( !h->_in_hash_table()); DBGTHRD(<<"h=" << *h);
			...
			else
			{
				...
				fprintf(
						...
						h->refcount, sd, smlevel_0::in_recovery());
				// croak here
 ...
 */
int histoid_t::__find_page_in_heap(bool insert_if_not_found, const shpid_t& pid)
{
	...
	while (1)
	{
		...
		if (insert_if_not_found)
		{
			...
			w_assert3(_have_mutex()); DBG(<<"insert if not found");
			install(info);
 ...
 *              
 ********************************************************************/
void histoid_t::_find_page_return_info(bool init_pginfo_if_not_found,
		const shpid_t& pid, bool& found_in_table, pginfo_t& result)
{
	...
	else if (hook >= 0 /*found_in_table*/)
	{
		...
		DBGTHRD(<<"this " << this << " _find_page_return_info, pgheap.RemoveN page "
				<<result.page()
				<< " space " << result.space()); w_assert3(pgheap->HeapProperty(0));
	}
#if W_DEBUG_LEVEL > 1
	if(found_in_table)
 ...
 */
w_rc_t histoid_t::find_page(smsize_t space_needed, bool& found, pginfo_t& info,
		page_p* pagep /* input */, slotid_t& idx // output iff found
) const
{
	...
	while (!found)
	{
		...
		{
			// legit
			w_assert3(_have_mutex()); w_assert3(pgheap->HeapProperty(0));
			info = pgheap->RemoveN(hook);
			w_assert3(pgheap->HeapProperty(0)); DBGTHRD(<<"pgheap.RemoveN page " <<info.page() << " space " << info.space());
			...
			if (success)
			{
				...
				W_DO(latch_lock_get_slot(pg, pagep, space_needed,
								true, // not append-only // hpl changed back to true in order to remove staircase effect
								success, idx));
				if (success)
				{
					// checking here ONLY so we can tell the path taken
					w_assert2(pagep->pid().page == pg); w_assert2(pagep->pid().stid().store == cmp.key.store);
			...
			if (success)
			{
				...
				w_assert2(idx != 0); w_assert2(cmp.key.store != 0);
		...
		else
		{
			...
			DBGTHRD(<<"histoid_t::find_page NOT FOUND in cache"); w_assert3(!found);
			return RCOK;
		}
	} // while !found
...
w_rc_t histoid_t::latch_lock_get_slot(shpid_t& shpid, page_p* pagep,
		smsize_t space_needed, bool append_only, bool& success, slotid_t& idx // only meaningful if success
) const
{
	success = false;
	...
	rc_t rc = ((page_p *) pagep)->conditional_fix(pid, page_p::t_any_p, LATCH_EX, 0, junk, true);
	...
	if (rc.is_error())
	{
		...
		if (rc.err_num() == smthread_t::stINUSE || rc.err_num() == smthread_t::stTIMEOUT)
		{
	...
	else
	{
		...
		rc = pagep->_find_and_lock_free_slot(append_only, space_needed, idx);
		DBGTHRD(<<"rc=" <<rc);
...
void histoid_t::install(const pginfo_t &info)
{
	DBGTHRD("install info in heap" << info); w_assert1( _in_hash_table() );
 ...
 * there before.
 */
void histoid_t::update_page(const shpid_t& pid, smsize_t amt)
{
	DBGTHRD(<<"update_page");
	_grab_mutex();
...
w_rc_t histoid_t::exists_page(smsize_t space_needed, bool& found) const
{
	space_bucket_t b = page_p::free_space2bucket(space_needed);
	_grab_mutex();
	// DBGTHRD(<<"histogram for store " << cmp.key << "=" << histogram << endl );
	DBGTHRD(<<"exists_page store " << cmp.key);
	while (!(found = histogram.exists(b)) && b < (space_num_buckets - 1))
		b++;
	_release_mutex();
	return RCOK;
}

void histoid_t::bucket_change(smsize_t old_space_left, smsize_t new_space_left)
{
	space_bucket_t ob = page_p::free_space2bucket(old_space_left);
	space_bucket_t nb = page_p::free_space2bucket(new_space_left);
	if (ob != nb)
	{
		_grab_mutex();
		histogram.decr(ob);
...
{
	o << this;
	o << " key=" << cmp.key << " refcount=" << refcount;
	if (_in_hash_table())
		o << ", hashed";
...
histoid_remove_t::histoid_remove_t(page_p &pg) :
			...
			_page(&pg), _h(NULL)
{
	...
	w_assert2(_h->refcount == 0); w_assert1(! _h->_in_hash_table());
	...
	w_assert1(! _h->_in_hash_table()); w_assert1( !_h->_have_mutex());
	delete _h;
}
...
{
	DBGTHRD(<<"~histoid_update_t"); DBGTHRD(<<"DESTRUCT histoid_update_t: " << *this);
	if (_h)
	{
		if (_found_in_table)
		{
...
void histoid_update_t::replace_page(page_p *p, bool reinstall)
{
	if (_page)
	{
	...
	if (differs || p != _page)
	{
		...
		{
			w_assert3(!_h->_have_mutex()); DBGTHRD(<<"bucket change for page " << _info.page());
			if (_page && _page->is_fixed())
			{
				w_assert3(_info.page() == _page->pid().page);
			}
...
void histoid_update_t::remove()
{
	...
	if (_page && _page->pid().page)
	{
		...
		_h->_find_page_return_info(false, _page->pid().page, _found_in_table,
				_info);
		w_assert2(!_found_in_table); w_assert2(_info.page() == _page->pid().page);
	} DBGTHRD(<<"remove page " << _page->pid().page
			<< " refcount= " << _h->refcount);
	_page = 0;
	_found_in_table = 0;
	DBGTHRD(<<"bucket change for page " << _info.page()); w_assert2(_h);
...
void histoid_update_t::update()
{
	// called when page is updated
	w_assert3(_h); w_assert3(_h->refcount > 0); w_assert3(_page->is_fixed()); w_assert3(_page->latch_mode() == LATCH_EX);
	...
	DBGTHRD(<<"bucket change for page " << _info.page()); w_assert3(_info.page() == _page->pid().page);
...
bool histoid_compare_t::match(const pginfo_t& left, const pginfo_t& right) const
{
...
bool histoid_compare_t::gt(const pginfo_t& left, const pginfo_t& right) const
{
...
bool histoid_compare_t::ge(const pginfo_t& left, const pginfo_t& right) const
{
	// return true if left >= right
...
static __thread queue_based_lock_t::ext_qnode histoid_me =
QUEUE_EXT_QNODE_INITIALIZER;

void histoid_t::_release_mutex() const
{
	w_assert2(_histoid_mutex.is_mine(&histoid_me));
	_histoid_mutex.release(&histoid_me);
}
void histoid_t::_grab_mutex() const
{
	w_assert2( ! _histoid_mutex.is_mine(&histoid_me));
	_histoid_mutex.acquire(&histoid_me);
}
void histoid_t::_grab_mutex_cond(bool& got) const
{
	w_assert2( ! _histoid_mutex.is_mine(&histoid_me));
	got = _histoid_mutex.attempt(&histoid_me);
}

bool histoid_t::_have_mutex() const
{
