...
rc_t pin_i::pin(const rid_t& rid, smsize_t start, lock_mode_t lmode, latch_mode_t latch_mode)
{
...
rc_t pin_i::update_rec(smsize_t start, const vec_t& data, int* old_value /* temporary: for degugging only */
)
{
	...
	if (was_pinned && _rec->is_small())
	{
		...
		w_assert3(_hdr_page().latch_mode() == LATCH_EX); w_assert3((_lmode == EX)
		);
		...
		W_DO_GOTO(rc,
				_hdr_page().splice_data(_rid.slot, u4i(start), data.size(), data));
