 ...
 *
 *********************************************************************/
rc_t page_p::_fix(bool condl, const lpid_t& pid, tag_t ptag, latch_mode_t m,
		uint4_t page_flags, store_flag_t& store_flags,//used only if page_flags & t_virgin
		bool ignore_store_id, int refbit)
{
	...
	if (_pp && _pp->pid == pid)
	{
		...
		else if (condl)
		{
			...
			if (would_block)
				return RC(sthread_t::stINUSE);w_assert2(_pp && bf->is_bf_page(_pp, true));
	...
	else
	{
		...
		if (condl)
		{
			W_DO( bf->conditional_fix(_pp, pid, ptag, m,
							...
							ret_store_flags,
							ignore_store_id, store_flags) );w_assert2(_pp && bf->is_bf_page(_pp, true));
		}
		else
		{
	...
	w_assert2(is_fixed());w_assert2(_pp && bf->is_bf_page(_pp, true));
	return RCOK;
}
...
rc_t page_p::next_slot(uint4_t space_needed, slotid_t& ret_idx)
{
...
/*	if (_pp->nslots == 199)
	...
	if (usable_space_for_slots() < sizeof(slot_t) || usable_space() < space_needed)
	{
		return RC(eRECWONTFIT);
	}
 ...
 *********************************************************************/
rc_t
page_p::insert_expand(slotid_t idx, int cnt, const cvec_t *vec, bool log_it, bool do_it)
{
...
#if HPL_DEBUG>1
        //cout<<"Z splice avoid saving old="<< (len - osave)<< " new= "<< (vec.size()-nsave)<<endl;
#endif
        rc = log_page_splicez(*this, idx, start, len, osave, nsave, vec);
   } else {
#if HPL_DEBUG>1
        //cout<<"log splice idx=" <<  idx << " start=" << start << " len=" << len <<endl;
#endif
		rc = log_page_splice(*this, idx, start, len, vec);
    }
    if (rc.is_error())  {
#if HPL_DEBUG>0
        cout<<"LOG FAILURE rc=" << rc <<endl;
#endif
