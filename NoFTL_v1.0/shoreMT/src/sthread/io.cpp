...
sthread_t::open(const char* path, int flags, int mode, int &ret_fd)
{
    ...
    if (open_local) {
    	if (sdisk_nand_t::isFlashsimDevice(path))
    		e = sdisk_nand_t::make(path, flags, mode, dp);
    	else
...
w_rc_t    sthread_t::erase_block(int fd, int blk_num)
{
    fd -= fd_base;
    if (fd < 0 || fd >= (int)open_max || !_disks[fd])
        return RC(stBADFD);
    ...
    e = _disks[fd]->erase_block(blk_num);
    return e;
}

w_rc_t    sthread_t::read_oob(int fd, int psn, int offset, int length, void *buf, bool with_delay)
{
    fd -= fd_base;
    if (fd < 0 || fd >= (int)open_max || !_disks[fd])
        return RC(stBADFD);
    ...
    e = _disks[fd]->read_oob(psn, offset, length, buf, with_delay);
    return e;
}

w_rc_t    sthread_t::write_oob(int fd, int psn, int offset, int length, void *buf, bool with_delay)
{
    fd -= fd_base;
    if (fd < 0 || fd >= (int)open_max || !_disks[fd])
        return RC(stBADFD);
    ...
    e = _disks[fd]->write_oob(psn, offset, length, buf, with_delay);
    return e;
}
