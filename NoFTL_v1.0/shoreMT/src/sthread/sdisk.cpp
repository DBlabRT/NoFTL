...
//Relevant only for raw NAND devices
w_rc_t    sdisk_t::erase_block(int blk_num)
{
	return RC(fcNOTIMPLEMENTED);
}

w_rc_t    sdisk_t::read_oob(int psn, int offset, int length, void *buf, bool with_delay)
{
	return RC(fcNOTIMPLEMENTED);
}

w_rc_t    sdisk_t::write_oob(int psn, int offset, int length, void *buf, bool with_delay)
{
	return RC(fcNOTIMPLEMENTED);
}
/**\endcond skip */
